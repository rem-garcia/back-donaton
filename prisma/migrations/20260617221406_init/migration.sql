...
CREATE TABLE `Donante` (
    ...
    `ot` VARCHAR(191) NOT NULL,
    `creacion` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),

    UNIQUE INDEX `Donante_correo_key`(`correo`),
    UNIQUE INDEX `Donante_ot_key`(`ot`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Donacion` (
    ...
    `estado` ENUM('RECIBIDA', 'DISPONIBLE', 'ASIGNADA', 'EN_TRANSITO', 'ENTREGADA') NOT NULL DEFAULT 'RECIBIDA',
    `fecha` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
